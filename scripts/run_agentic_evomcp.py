#!/usr/bin/env python3
"""Run local evomcp EvoX with the ecdsa.fail agentic evaluator."""
...
def main() -> None:
    ...
    from optim.evaluators.agentic_ecdsafail import AgenticEcdsaFailEvaluator

    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    archive = run(args.config, evaluator=AgenticEcdsaFailEvaluator(project_root), resume=args.resume)
    best = archive.best()
    if best:
